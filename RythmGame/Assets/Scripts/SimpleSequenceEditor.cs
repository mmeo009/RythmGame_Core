...
public class SimpleSequenceEditor : EditorWindow
{
    ...
    private int currentBeatTime = 0;                    // 현재 재생중인 비트 시간
    ...
    private void OnEnable()                                 // 윈도우 에디터 창을 열었을 때
    {
        ...
        CreateAudioSource();                                // 오디오를 사용할 것이기 때문에 오디오 등록을 해줌
    }

    private void OnDisable()                                // 윈도우 에디터 창을 열었을 때
    {
    ...
    private void CreateAudioSource()                        // 유니티 Scene에서 처럼 오디오 소스를 사용 하기 위해 보이지 않게 등록
    {
    ...
    private void InitializeTracks()                         // 시퀸스 데이터가 들어왔을 때 데이터를 셋팅해주는 함수
    {
        if (sequenceData == null) return;                   // 데이터가 없을 경우 그냥 리턴

        if(sequenceData.trackNotes == null)                 // 데이터는 있는데 트렉 노트가 없을 경우 자료형을 생성 시켜준다.
        {
        ...
        foreach(var track in sequenceData.trackNotes)       // 각 트랙 노트의 비트 수 만큼 0 값으로 넣어 준다.
        {
        ...
        if(audioSource != null)                             // 가져온 시퀀스 데이터의 오디오 클립이 있을 경우 오디오 데이터를 할당해 준다.
        {
    ...
    private void Update()
    {
        ...
        if(isPlaying && audioSource != null && audioSource.isPlaying)
        {
            ...
            currentBeatTime = Mathf.FloorToInt(elapseTime * sequenceData.bpm / 60f);        // BPM분의 60에 흐른시간을 곱하면 현재 비트다.

            if(currentBeatTime >= totalBeats)
            {
                StopPlayBack();
            }
            Repaint();                                      // 업데이트가 끝난 후에 리페인트 (화면 갱신을 해주지 않는 업데이트이기 때문)
        }
    }
    private void StartPlayBack(int fromBeat)                // 오디오를 특정 비트부터 플레이 해 주는 함수
    {
    ...
    private void DrawBeat(int trackIndex, int beatIndex)
    {
        ...
        // 현재 트랙의 현재 비트 인덱스에 해당하는 노트값을 가져오면서 검사해서 유효한 인덱스가 아닐경우 0으로 설정한다.
        ...
        {
            if(Event.current.button == 0)           // 왼쪽 클릭
            {
            ...
            else if(Event.current.button == 1)      // 오른쪽 클릭
            {
    ...
    private void OnGUI()
    {
        ...
        {
            sequenceData.numberOfTracks = newNumberOfTracks;            // 트랙 수를 가져와서 갱신 시킨다.
            InitializeTracks();
        }
        ...
        {
            if (isPlaying) PausePlayBack();         // 재생중일 경우 멈춤
            else StartPlayBack(currentBeatTime);    // 일시 정지일 경우 다시 재생
        }
        ...
        GUILayout.Space(10);            // UI사이에 공간 10픽셀
